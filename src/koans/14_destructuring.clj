...
(meditations
  ...
  (= (str "First comes love, "
     ...
     ((fn [[a b c]] 
        (format (str "First comes love, " "then comes marriage, "
                     "then comes Clojure with the baby carriage") a b c))
  ...
  (= "Rich Hickey aka The Clojurer aka Go Time aka Macro Killah"
     (let [[first-name last-name & aliases]
       ...
       (apply str (interpose " aka " (list* (str first-name " " last-name) aliases)))))
  ...
  (= "Test Testerson, 123 Test Lane, Testerville, TX"
     ((fn [[f l] {:keys [street-address city state]}]
        (str f " " l ", " (apply str (interpose ", " (list street-address city state))))) 
